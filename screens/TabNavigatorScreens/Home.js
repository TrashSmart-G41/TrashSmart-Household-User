...
<View className='flex-row flex-1 bg-white py-2 px-4 my-2 rounded-lg' >
  ...
  <View className='ml-2' >
    ...
    <Text className='text-gray-500 text-base font-normal leading-none mt-[-3]' >
        Truck No: <Text className='font-bold' >{truckNo}</Text></Text>
...
const Home = ({ navigation }) => {
  const renderNewDispatchesItem = ({ item }) => <NewDispatchesItem date={item.date} truckNo={item.truckNo} address={item.address} />;
  const renderPastDispatchesItem = ({ item }) => <PastDispatchesItem date={item.date} truckNo={item.truckNo} address={item.address} />;
